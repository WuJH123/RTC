...
def test_query_margin_decomposition_is_invariant_to_common_rank_shift()->None:
    ...
    _zero(adapter)
    context=torch.zeros(11); candidate=torch.zeros((3,9))
    first=adapter(raw_rank_scores_m3=torch.tensor([30.0,-20.0,10.0]),context_features=context,candidate_features=candidate)
    shifted=adapter(raw_rank_scores_m3=torch.tensor([5030.0,4980.0,5010.0]),context_features=context,candidate_features=candidate)
    assert torch.allclose(first.relative_rank_normalized,shifted.relative_rank_normalized,atol=1e-6)
    assert int(torch.argmin(first.predicted_returns_m3))==1
    assert float(first.predicted_returns_m3[1])==pytest.approx(float(first.query_best_margin_m3))
...
def test_query_margin_sign_is_separate_from_candidate_rank()->None:
    ...
    out=adapter(raw_rank_scores_m3=torch.tensor([100.0,-50.0]),context_features=torch.zeros(11),candidate_features=torch.zeros((2,9)))
    ...
    adapter.margin_head[-1].bias.data.fill_(0.25)
    hold=adapter(raw_rank_scores_m3=torch.tensor([100.0,-50.0]),context_features=torch.zeros(11),candidate_features=torch.zeros((2,9)))
...
def test_query_feature_contract_enforces_82_of_109_and_three_families()->None:
    mask=np.zeros(109,dtype=bool); mask[:82]=True
    active=torch.zeros(109); targets=torch.zeros((2,109)); targets[0,0]=0.2; targets[1,1]=0.3
    context,candidates=build_query_margin_features(
        current_state=torch.zeros((4,3)),rainfall_scenarios=torch.zeros((3,72,4,1)),
        previous_actuator_flow=torch.zeros(109),active_target=active,candidate_targets=targets,
        base_step2_scores_m3=torch.tensor([-10.0,-20.0]),
        candidate_sources=["STEP2_H10_PROBE_SCALE_0.50","TYPE_AWARE_HYDRAULIC_PRESSURE"],
        supervisory_mask=mask,target_scale_m3=1000.0,
    )
    assert tuple(context.shape)==(11,); assert tuple(candidates.shape)==(2,9)
    bad=targets.clone(); bad[0,100]=0.5
    with pytest.raises(ValueError,match="passive"):
        build_query_margin_features(
            current_state=torch.zeros((4,3)),rainfall_scenarios=torch.zeros((3,72,4,1)),
            previous_actuator_flow=torch.zeros(109),active_target=active,candidate_targets=bad,
            base_step2_scores_m3=torch.tensor([-10.0,-20.0]),
            candidate_sources=["STEP2_H10_PROBE_SCALE_0.50","TYPE_AWARE_HYDRAULIC_PRESSURE"],
            supervisory_mask=mask,target_scale_m3=1000.0,
        )
...
def test_runtime_ranks_before_conformal_and_keeps_policy_lock_closed()->None:
    ...
    assert text.index("selected_index=int(np.argmin(relative))") < text.index("upper=score+margin")
    ...
    assert DIRECT_TFV_QUERY_MARGIN_CONTRACT in __import__("rtc.direct_tfv_policy_return_query_margin",fromlist=["DIRECT_TFV_QUERY_MARGIN_CONTRACT"]).DIRECT_TFV_QUERY_MARGIN_CONTRACT
