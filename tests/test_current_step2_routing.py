...
def test_current_contract_is_direct_tfv_core_v5() -> None:
    payload = json.loads(CURRENT.read_text(encoding="utf-8"))
    assert payload["contract"] == "PROJECT7_CURRENT_DIRECT_TFV_CONTROL_V5"
    ...
    assert payload["step3_contract"] == "PROJECT7_DIRECT_TFV_109ACT_RECEDING_MPC_V4"
    assert payload["step3_current"]["admission"].startswith("no separate calibrated threshold")
    assert "q95" in payload["step3_current"]["active_set"]
...
def test_execution_registry_routes_directly_from_step2_to_step3() -> None:
    ...
    assert current["research_contract"] == "PROJECT7_CURRENT_DIRECT_TFV_CONTROL_V5"
    assert current["training_contract"] == "PROJECT7_DIRECT_TFV_CORE_TRAINING_V5"
    assert current["step3_contract"] == "PROJECT7_DIRECT_TFV_109ACT_RECEDING_MPC_V4"
    ...
    assert "density-balanced" in selected["step2_training"]
    assert selected["step3_development"] == "src/rtc/step3_tfv_value_mpc_v4.py"
...
def test_current_cli_surfaces_are_explicit_and_threshold_free() -> None:
    ...
    assert "--active-support-quantile" in step3
    runtime = _help(DEV_RUNTIME)
    for argument in ("--inp", "--step1", "--step2", "--sensors", "--active-facilities", "--active-support-quantile"):
...
def test_current_lint_surface_tracks_core_paths() -> None:
    ...
    assert payload["contract"] == "PROJECT7_CURRENT_LINT_SURFACE_DIRECT_TFV_V8"
    ...
    required = {
        ...
        "tests/test_direct_tfv_competitive_v5.py",
