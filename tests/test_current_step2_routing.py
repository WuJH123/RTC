...
def test_current_contract_is_fresh_admission_direct_tfv_v8() -> None:
    payload = json.loads(CURRENT.read_text(encoding="utf-8"))
    assert payload["contract"] == "PROJECT7_CURRENT_DIRECT_TFV_CONTROL_V8"
    ...
    assert payload["fresh_admission_data_contract"] == "PROJECT7_DIRECT_TFV_FRESH_D3_HOLD_ADMISSION_CALIBRATION_V1"
    assert payload["sequence_support_contract"] == "PROJECT7_DIRECT_TFV_D3_HOLD_JOINT_SEQUENCE_SUPPORT_V1"
    assert payload["admission_data_contract"]["minimum_fresh_calibration_rainfall_groups"] == 9
    assert payload["admission_data_contract"]["step2_trainfit_role"] == "MODEL_FIT_ONLY_NEVER_CONFORMAL_CALIBRATION"
    ...
    assert payload["scientific_bottleneck"]["classification"] == "ADMISSION_CALIBRATION_FAIL_CLOSED_INSUFFICIENT_RAINFALL_GROUPS"


def test_execution_registry_routes_fresh_preflight_before_admission_and_step3() -> None:
    payload = json.loads(REGISTRY.read_text(encoding="utf-8"))
    current = payload["current"]
    assert payload["contract"] == "PROJECT7_EXECUTION_REGISTRY_DIRECT_TFV_V9"
    assert current["research_contract"] == "PROJECT7_CURRENT_DIRECT_TFV_CONTROL_V8"
    ...
    assert current["fresh_admission_preflight"] == "scripts/validate_direct_tfv_fresh_admission_data_current.py"
...
def test_current_cli_surfaces_expose_fresh_calibration_and_optimizer_consistent_step3() -> None:
...
def test_current_lint_surface_tracks_fresh_admission_paths() -> None:
    payload = json.loads(LINT.read_text(encoding="utf-8"))
    assert payload["contract"] == "PROJECT7_CURRENT_LINT_SURFACE_DIRECT_TFV_V11"
    paths = set(payload["paths"])
    required = {
        "scripts/build_direct_tfv_sequence_support_current.py",
        "scripts/validate_direct_tfv_fresh_admission_data_current.py",
        ...
        "src/rtc/direct_tfv_admission_split.py",
        ...
        "tests/test_direct_tfv_fresh_admission_split.py",
