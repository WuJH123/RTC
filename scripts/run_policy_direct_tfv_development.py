"""Run optimizer-consistent Direct-TFV MPC in an authoritative Development SWMM closed loop."""
...
DIRECT_TFV_DEVELOPMENT_RUNTIME_CONTRACT = "PROJECT7_DIRECT_TFV_AUTHORITATIVE_10MIN_DEVELOPMENT_RTC_V5"
...
def main() -> None:
    ...
    controller_cfg = replace(
        controller_cfg, horizon_steps=72, control_block_steps=2,
    ...
    design = DirectTFVMPCDesignV4(
        maxiter=int(args.lbfgsb_maxiter), deadline_seconds=float(args.optimizer_deadline_seconds),
        active_facility_count=int(args.active_facilities), active_support_quantile=str(args.active_support_quantile),
    )
    mpc = DirectTFVRecedingMPCV6(
        model=model, graph=graph, normalization=normalization,
        action_support=checkpoint["action_support"], admission_calibration=admission,
        sequence_support=sequence_support, design=design,
    )
    ...
    runtime_inp = _controls_disabled_runtime(
        source_inp=Path(args.inp), cache_dir=Path(args.out_dir) / "_runtime_inp",
        swmm_threads=int(cfg.get("swmm_threads", 1)),
    )
    result = run_authoritative_closed_loop(
        inp_path=runtime_inp, output_dir=args.out_dir, run_id=args.run_id,
        sensor_nodes=sensors, controller=controller,
        control_start_minutes=int(cfg["control_start_minutes"]), control_update_seconds=600,
        observation_update_seconds=300, record_stride_seconds=300, exact_global_peak=False,
    )
    ...
    metadata.update({
        "strategy": "proposed_direct_tfv_optimizer_consistent_receding_mpc",
        "direct_tfv_development_runtime_contract": DIRECT_TFV_DEVELOPMENT_RUNTIME_CONTRACT,
        "development_only": True, "tfv_primary": True,
        "all_109_facilities_screened_each_decision": True, "dynamic_active_set": True,
        ...
        "action_rule": "score support-contracted H120 sequence; execute iff calibrated upper bound on delta TFV < 0 and first move changes",
        ...
        "free_control_horizon_minutes": 120, "prediction_horizon_minutes": 360,
        "execute_first_move_minutes": 10, "future_realized_rainfall_used_as_model_input": False,
        "score_equals_execute_required": True, "target_write_readback_audit": write_audit,
        "step2_model_sha256": _sha(args.step2), "step2_training_contract": str(checkpoint.get("training_contract", "")),
        "graph_schema_sha256": _sha(args.graph), "step1_model_file_sha256": _sha(args.step1),
        "sensor_layout_file_sha256": _sha(args.sensors), "source_inp_sha256": _sha(args.inp),
        "controller_config_sha256": _sha(args.config), "project7_runtime_contract": project_contract,
        "prepared_event_clock": event_clock,
    })
    metadata_path.write_text(json.dumps(metadata, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps({
        "strategy": metadata["strategy"], "runtime_contract": DIRECT_TFV_DEVELOPMENT_RUNTIME_CONTRACT,
        "metadata_path": result.metadata_path, "decision_path": result.decision_path,
        "node_statistics_path": result.node_statistics_path, "decisions": result.decisions,
        ...
        "joint_sequence_support_sha256": metadata["joint_sequence_support_sha256"],
