"""Run Proposed V25 on the frozen Operational Benchmark5 and reuse immutable baselines."""
...
def _run_proposed_v25(
    *,
    ...
    if metadata_path.exists():
        raise FileExistsError(f"V25 Proposed result already exists for {event_id}; use a fresh --out-dir")
    event_root.mkdir(parents=True, exist_ok=True)
    command = [
        sys.executable,
        str(Path(__file__).resolve().parent / "run_policy_direct_tfv_operational_v25_development.py"),
        ...
        "--v25-value-checkpoint", str(v25_value),
    ...
    if metadata.get("development_only") is not True or metadata.get("formal_evidence") is not False:
        raise RuntimeError("V25 operational run lost Development-only firewall")
    if metadata.get("v24_hydraulic_stress_escape_enabled") is not False:
        raise RuntimeError("V25 metadata still enables the retired V24 escape")
    if metadata.get("hydraulic_stress_is_diagnostic_only") is not True:
        raise RuntimeError("V25 metadata did not keep stress diagnostic-only")
    if str(metadata.get("source_inp_sha256", "")).lower() != str(event["inp_sha256"]).lower():
        raise RuntimeError("V25 Proposed run used another event INP")
...
def main() -> None:
    ...
    # Keep the base runner's public options and add only V25 lineage assets.
    ...
    parser.add_argument("--v25-value-checkpoint", required=True)
    ...
    base._run_proposed = lambda **kwargs: _run_proposed_v25(
        **kwargs,
        v25_value=Path(args.v25_value_checkpoint).resolve(),
        matched_bank_manifest=Path(args.matched_truth_bank_manifest).resolve(),
    )
    original_argv = list(sys.argv)
    try:
        # Reuse the audited base aggregation without rebuilding the baseline cache.  Only the
        # Proposed callback is replaced; the base parser receives no unknown V25-only options.
        sys.argv = [
