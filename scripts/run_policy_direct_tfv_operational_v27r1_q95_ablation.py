"""Run one Development-only Project7 V27R1 physical-only q95 ablation event."""
...
from rtc.direct_tfv_operational_v27r1_runtime import (
...
def main() -> None:
    ...
    controller, _, sensors, lineage = build_operational_v27r1_controller(
    ...
    if write_audit.get("passed") is not True:
        raise RuntimeError("operational V27R1 q95 ablation failed target write/readback audit")

    metadata_path = Path(result.metadata_path)
    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    metadata.update(
        {
            "strategy": "proposed_v27r1_q95_ablation",
            "operational_development_runtime_contract": OPERATIONAL_V27R1_RUNTIME_CONTRACT,
