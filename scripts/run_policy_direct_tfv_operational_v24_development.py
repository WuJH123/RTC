"""Run Development-only V24 RBC-informed discriminative RTC in authoritative SWMM.

V24 preserves frozen Step1/Direct-TFV V5/V15/V21 and the V23 candidate generator.  It changes only
high-stress learned HOLD handling: when causal reconstructed network stress reaches the frozen
Auto-RBC high-fill threshold, the existing type-aware hydraulic candidate is used as an engineering
stress escape.  This runner is Development-only; historical V23 Formal/Final evidence is untouched.
"""
...
def main() -> None:
    ...
    controller, _graph, sensors, lineage = build_operational_v24_controller(
    ...
    if write_audit.get("passed") is not True:
        raise RuntimeError("operational V24 failed target write/readback audit")

    metadata_path = Path(result.metadata_path)
    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    metadata.update(
        {
            "strategy": "proposed_v24_rbc_informed",
            "operational_development_runtime_contract": OPERATIONAL_V24_RUNTIME_CONTRACT,
            "hybrid_admission_contract": V24_HYBRID_ADMISSION_CONTRACT,
            "development_only": True,
            "formal_evidence": False,
            "historical_v23_evidence_mutated": False,
            ...
            "hold_allowed_low_moderate_stress": True,
            "high_stress_hold_escape_enabled": True,
            "high_stress_escape_q75": V24_STRESS_ESCAPE_Q75,
            "high_stress_escape_blend_minimum": V24_STRESS_ESCAPE_BLEND_MIN,
