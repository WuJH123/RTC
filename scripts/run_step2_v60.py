...
def _build_models(graph: GraphSchema, cache: V60TrainCache, fit_names: list[str], scales) -> tuple[ControlValueSurrogateV60, HydraulicResponseSurrogateV60]:
    ...
    value = ControlValueSurrogateV60(tfv_rate_scale_m3s=scales.tfv_rate_scale_m3s, **common)
...
def _coefficient_gradient_audit(value, graph, basis, cache, name, norm, device: str) -> dict[str, float]:
    ...
    seed = torch.zeros(
        1, 4, basis.temporal_basis_count, basis.group_count,
        dtype=reference.dtype, device=target,
    )
    for c in range(4):
        seed[0, c, c % basis.temporal_basis_count, (3 * c) % basis.group_count] = 0.5 if c % 2 == 0 else -0.5
    coeff = seed.requires_grad_(True)
    reference_expanded = reference[:, None].expand(1, 4, -1, -1)
    candidate = basis.decode(reference_expanded, coeff)
    output = value(
        batch.initial_state, batch.rainfall, batch.reference_settings,
        candidate, prepared, batch.elapsed_seconds,
    )
    ...
    return {
        ...
        "nonzero_fraction": float((grad.abs() > 1e-12).float().mean()),
        "median_abs": float(grad.abs().median()),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run bounded Project7 Step2 V6 dual-surrogate Train-only development")
    parser.add_argument("--graph", required=True)
    parser.add_argument("--cache-manifest", required=True, help="dedicated V6 D2 + targeted D3-v2 cache manifest")
    ...
    if len(graph.actuator_ids) != 109:
        raise ValueError(f"V6 frozen actuator contract requires 109, got {len(graph.actuator_ids)}")
    cache = V60TrainCache(args.cache_manifest)
    d2 = cache.names("D2")
    targeted_d3 = cache.targeted_d3_names()
    legacy_d3 = cache.legacy_d3_names()
    if not d2:
        raise ValueError("V6 cache contains no D2 groups")
    if not targeted_d3:
        raise ValueError(
            "V6 cache contains no targeted D3-v2 groups. Design/run D3-v2 and build the dedicated V6 index/cache first."
        )
    if legacy_d3:
        raise ValueError(
            f"V6 dedicated cache contains {len(legacy_d3)} legacy dense D3 groups; rebuild from D2 + targeted D3-v2 only"
        )
    selected = sorted(d2 + targeted_d3)
    ...
    prepared = prepare_static_v60(graph, target)
    value_metrics = {
        "fit_d2": evaluate_value_v60(value, cache, fit_d2, norm, prepared, device=target),
        "fit_d3": evaluate_value_v60(value, cache, fit_d3, norm, prepared, device=target),
        "holdout_d2": evaluate_value_v60(value, cache, holdout_d2, norm, prepared, device=target),
        "holdout_d3": evaluate_value_v60(value, cache, holdout_d3, norm, prepared, device=target),
    }
    hydraulic_metrics = {
        "holdout_d2": evaluate_hydraulic_v60(hydraulic, cache, holdout_d2, norm, graph, device=target),
        "holdout_d3": evaluate_hydraulic_v60(hydraulic, cache, holdout_d3, norm, graph, device=target),
    }
    basis = build_control_basis_v60(graph)
    gradient = _coefficient_gradient_audit(
        value, graph, basis, cache, holdout_d3[0], norm, str(target)
    )
    ...
    report = {
        ...
        "fit_d2": len(fit_d2), "fit_d3": len(fit_d3),
        "holdout_d2": len(holdout_d2), "holdout_d3": len(holdout_d3),
        "legacy_dense_d3_present": False,
        "control_basis": basis_manifest_v60(basis),
        "value_metrics": value_metrics,
        "hydraulic_metrics": hydraulic_metrics,
        "control_latent_gradient": gradient,
