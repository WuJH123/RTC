"""Run V21 as a Development-only operational RTC probe in authoritative SWMM.

Unlike the Formal promotion path, this runner may use a V21 checkpoint that failed the offline AUC
gate. The purpose is development steering on the frozen five-event benchmark, not scientific
promotion. Causality, 82/109 geometry, q95 support, H10 execution, target write/readback and no-online-
SWMM-search constraints remain mandatory.
"""
...
def main() -> None:
    ...
    controller, _graph, sensors, lineage = build_operational_v21_controller(
    ...
    if write_audit.get("passed") is not True:
        raise RuntimeError("operational V21 failed target write/readback audit")

    metadata_path = Path(result.metadata_path)
    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    metadata.update({
        "strategy": "proposed",
        "operational_development_runtime_contract": OPERATIONAL_V21_RUNTIME_CONTRACT,
