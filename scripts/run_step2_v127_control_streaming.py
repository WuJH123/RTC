"""Train Project7 V127 Step2 without materializing full counterfactual groups on CUDA.

Execution profile targets a 16-GB RAM / RTX 4060 8-GB workstation. Scientific data roles,
losses, epochs and evaluation semantics remain the V127 control-oriented contract; only the
CPU/GPU materialization strategy and microbatch execution budget change.
"""
...
from rtc.step2_train_v127_control import V127ControlTrainingDesign
...
V127_STREAMING_RUN_CONTRACT = (
    "PROJECT7_V127_EXISTING_DATA_CONTROL_STREAMING_V1_16GB_RAM_8GB_VRAM"
)
...
def _finite_metrics(name: str, metrics: dict[str, float | str]) -> None:
    ...
    if bad:
        raise RuntimeError(f"{name}: V127 streaming evaluation invalid for {bad}")
...
def _hardware(device: torch.device) -> dict[str, object]:
    payload: dict[str, object] = {
        "device": str(device),
        "streaming_cpu_group_gpu_microbatch": True,
        "amp_enabled": False,
    ...
    try:
        ...
        payload["system_ram_total_gb"] = float(vm.total / 1024**3)
        payload["system_ram_available_gb_at_start"] = float(vm.available / 1024**3)
    except Exception:
    ...
    if device.type == "cuda":
        ...
        if total < 7.0 * 1024**3:
            raise RuntimeError("V127 streaming profile expects an approximately 8-GB CUDA GPU")
        if free < 4.0 * 1024**3:
            raise RuntimeError(
                "less than 4 GB CUDA memory is free; close other GPU jobs before scientific training"
            )
    return payload
...
def main() -> None:
    ...
    if len(graph.actuator_ids) != 109:
        raise ValueError("V127 streaming control training requires exactly 109 actuators")
    cache_contracts = {
    ...
    state_scale, flow_scale, scale_telemetry = derive_residual_scales_streaming_v127(
        ...
        sample_rows=memory.residual_sample_rows,
    )
    ...
    hydraulic_history = train_hydraulic_stage_streaming_v127(
        model,
    ...
    rollout_history = train_truncated_rollout_stage_streaming_v127(
        model,
        source_caches=source_caches,
        source_groups=source_groups,
        normalization=normalization,
    ...
    objective_history = train_objective_stage_streaming_v127(
        model,
        source_caches=source_caches,
        source_groups=source_groups,
        normalization=normalization,
    ...
    report = {
        "contract": V127_STREAMING_RUN_CONTRACT,
        "training_contract": V127_STREAMING_TRAINING_CONTRACT,
        "step2_contract": model.v127_contract,
        "rtc_implementation_contract_sha256": rtc_implementation_contract_sha256(),
        "scientific_goal": (
            "control-oriented differentiable hydraulic response and trustworthy action gradients "
            "for 10-min continuous MPC"
        ),
        "hardware": hardware,
        "memory_execution": memory.__dict__,
