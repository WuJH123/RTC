"""Fast, mathematically equivalent V127 D2 gradient audit.

For a frozen D2 counterfactual group all actuator finite-difference directions share the
same causal state, rainfall and reference H72 action sequence. The legacy audit reran one
H360 autograd rollout per actuator direction. This version computes dJ/dU[72,109] once per
group and projects that full gradient onto every frozen direction in the group.
"""
...
from rtc.step2_train_v127_streaming import _cpu_group

V127_D2_FAST_GRADIENT_AUDIT_CONTRACT = (
    "PROJECT7_V127_INTERNAL_HOLDOUT_D2_GROUP_LOCAL_CAUSAL_GRADIENT_AUDIT_V4_FULL_GRADIENT_REUSE"
)
...
def _full_settings_gradient(
    ...
    *,
    ...
    settings = torch.as_tensor(
        base_sequence, dtype=initial.dtype, device=device
    ).requires_grad_(True)
    if settings.shape != (72, len(graph.actuator_ids)):
        raise ValueError("V127 D2 fast audit action sequence shape mismatch")
    ...
    if gradient.shape != settings.shape or not bool(torch.isfinite(gradient).all()):
        raise RuntimeError("V127 D2 fast audit produced invalid full settings gradient")
...
def main() -> None:
    ...
    if len(hold_d2) != 32:
        raise ValueError("V127 D2 fast audit requires frozen 32 InternalHoldout groups")
    ...
    for name in hold_d2:
        ...
        cpu = _cpu_group(online, name, normalization)
        ...
        for aid in sorted(set(aids.tolist()) - {""}):
            ...
            if direction_norm <= 1e-10:
                continue
            changed = np.flatnonzero(np.max(np.abs(direction), axis=0) > 1e-8)
    ...
    if detail.empty:
        raise RuntimeError("V127 D2 fast gradient audit produced no cases")
    ...
    metrics = {
        "contract": V127_D2_FAST_GRADIENT_AUDIT_CONTRACT,
        ...
        "predicted_gradient_semantics": "smooth V127 optimization TFV full dJ/dU projected along exact D2 action direction",
