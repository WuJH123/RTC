"""Run one Development-only information/action-authority matched baseline event.

This runner is intentionally separate from historical baseline caches.  It uses the Proposed sparse
sensor file, frozen Step1 reconstruction and the Proposed engineering/q95 execution envelope before
calling authoritative SWMM.  It must never overwrite or relabel native Auto-RBC/EFD/Internal evidence.
"""
...
def main() -> None:
    ...
    if write_audit.get("passed") is not True:
        raise RuntimeError(f"{args.strategy} failed target write/readback audit")
