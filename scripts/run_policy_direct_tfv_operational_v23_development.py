"""Run Development-only V23 strong-storm candidate steering in authoritative SWMM.

V23 changes only the existing hydraulic candidate target under broadly-high causal filling.  It keeps
frozen Step1/Step2/V15/V21, q95 support, three candidate families and the 0.5 target slew.  Candidate
distribution no longer exactly matches V21 training, so every result remains non-promotable
...
def main() -> None:
    ...
    if write_audit.get("passed") is not True:
        raise RuntimeError("operational V23 failed target write/readback audit")

    metadata_path = Path(result.metadata_path)
    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    metadata.update({
        "strategy": "proposed",
        "operational_development_runtime_contract": OPERATIONAL_V23_RUNTIME_CONTRACT,
