...
def main() -> None:
    ...
    design_source_shas = {str(value).lower() for value in candidates["rtc_source_tree_sha256"]}
    if design_source_shas != {current_source_sha.lower()}:
        raise ValueError(
            "first-move design was generated by a different RTC source tree; regenerate the panel "
            "before calibrating admission"
        )
    forbidden = (
        "T5",
        "T10",
    ...
    payload = {
        **calibrated,
        ...
        "panel_records": records,
        "lineage": {
            "rtc_source_tree_sha256": current_source_sha,
        ...
        "coverage_claim_scope": (
            ...
            "same source-fingerprinted shrink-only refined first-move optimizer used by Step3 V9"
        ),
    }
    out = Path(args.out)
