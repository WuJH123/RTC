"""Jointly score fresh calibration query sets with the decomposed policy-return critic.

Rows are grouped by query_set_id before scoring. Candidate ordering is relative to the other current
candidates in the same causal prefix, while the best-candidate margin is one query-level value. The
script only appends frozen predictions to existing authoritative calibration truth.
"""
...
def _load_context(
    row: dict[str, Any], *, expected_mask_sha: str
) -> dict[str, np.ndarray]:
    path = Path(str(row.get("context_npz", "")))
    if not path.is_file():
        raise ValueError(f"calibration context is missing: {path}")
    if sha256_file(path).lower() != str(row.get("context_npz_sha256", "")).lower():
        raise ValueError(f"calibration context SHA mismatch: {path}")
    ...
    context_mask = str(
        np.asarray(data["supervisory_mask_sha256"]).reshape(-1)[0]
    ).lower()
    if context_mask != expected_mask_sha:
...
def _rank_one(
    ...
    active = torch.as_tensor(
        context["active_target"], dtype=torch.float32, device=device
    )
    target = torch.as_tensor(
        context["candidate_target"], dtype=torch.float32, device=device
    )
    ...
    if int(rain_raw.shape[0]) != 1 or rain_raw.ndim != 5:
        raise ValueError(
            "calibration context must contain one candidate row with scenario rainfall"
        )
    ...
    state = ((state_raw - norm["state_mean"]) / norm["state_std"]).expand(
        scenarios, -1, -1
    )
    rain = ((rain_raw - norm["rain_mean"]) / norm["rain_std"]).reshape(
        scenarios, horizon, nodes, features
    )
    flow = ((flow_raw - norm["flow_mean"]) / norm["flow_std"]).expand(
        scenarios, -1
    )
    ...
    if not bool(torch.isfinite(score)):
        raise RuntimeError("query-margin calibration rank critic produced non-finite score")
    return score
...
def main() -> None:
    ...
    parser.add_argument("--query-margin-checkpoint", required=True)
    ...
    rank_model, normalization, adapter, checkpoint = load_query_margin_checkpoint(
        args.query_margin_checkpoint,
        graph=graph,
        base_step2_path=args.base_step2,
        device=device,
    )
    if checkpoint.get("fresh_validation_verified") is not True:
        raise ValueError(
            "calibration cannot score a critic that failed fresh validation"
        )
    _control, mask = load_native_supervisory_control(
    ...
    for line_number, raw in enumerate(
        ...
        if str(row.get("data_role")) != "policy_return_calibration":
            raise ValueError(
                "query-margin calibration scorer received another data role"
            )
        if str(row.get("supervisory_mask_sha256", "")).lower() != mask_sha:
            raise ValueError("calibration truth uses another supervisory mask")
        rows.append(row)
    if not rows:
        raise ValueError("query-margin calibration records are empty")

    by_query: dict[str, list[int]] = {}
    for index, row in enumerate(rows):
        by_query.setdefault(str(row["query_set_id"]), []).append(index)
    checkpoint_sha = sha256_file(args.query_margin_checkpoint)
    with torch.no_grad():
        for _query, positions in sorted(by_query.items()):
            if not 1 <= len(positions) <= 3:
                raise ValueError(
                    "current query-margin scorer requires 1--3 candidates per query"
                )
            contexts = [
                _load_context(rows[index], expected_mask_sha=mask_sha)
                for index in positions
            ]
            first = contexts[0]
            active0 = np.asarray(first["active_target"]).reshape(1, 109)[0]
            for context in contexts[1:]:
                other = np.asarray(context["active_target"]).reshape(1, 109)[0]
                if not np.allclose(other, active0, atol=1.0e-7):
                    raise ValueError(
                        "same query_set_id contains different active targets"
                    )
            raw_scores = torch.stack(
                [
                    _rank_one(
                        rank_model, normalization, graph, context, device
                    )
            ...
            base_scores = torch.as_tensor(
                [
                    float(
                        np.asarray(context["base_step2_h10_score_m3"])
                        .reshape(-1)[0]
                    )
                    for context in contexts
                ],
            ...
            context_features, candidate_features = build_query_margin_features(
                current_state=torch.as_tensor(
                    first["current_state"], dtype=torch.float32, device=device
                )[0],
                rainfall_scenarios=torch.as_tensor(
                    first["rainfall_scenarios"],
                    dtype=torch.float32,
                    device=device,
                )[0],
                previous_actuator_flow=torch.as_tensor(
                    first["previous_actuator_flow"],
                    dtype=torch.float32,
                    device=device,
                )[0],
                active_target=torch.as_tensor(
                    active0, dtype=torch.float32, device=device
                ),
                candidate_targets=candidate_targets,
                base_step2_scores_m3=base_scores,
                candidate_sources=[
                    str(rows[index]["candidate_source"]) for index in positions
                ],
                supervisory_mask=mask,
                target_scale_m3=float(
                    rank_model.target_scale_m3.detach().cpu()
                ),
            )
            output = adapter(
                raw_rank_scores_m3=raw_scores,
            ...
            relative_rank = (
                output.relative_rank_normalized.detach().cpu().numpy().astype(float)
            )
            margin = float(output.query_best_margin_m3.detach().cpu())
            for local, position in enumerate(positions):
                scored = dict(rows[position])
                scored["predicted_policy_return_delta_tfv_m3"] = float(
                    predictions[local]
                )
                scored["query_conditioned_best_candidate_margin_m3"] = margin
                scored["query_conditioned_relative_rank_normalized"] = float(
                    relative_rank[local]
                )
    ...
    summary = {
        ...
        "query_set_count": len(by_query),
        "rainfall_group_count": len(
