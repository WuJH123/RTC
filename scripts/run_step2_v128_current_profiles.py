...
CURRENT_RUN_CONTRACT = "PROJECT7_V128_CURRENT_SMOKE_DEV_FULL_STAGED_V2"
...
def _cache_time_engine(path: str | Path) -> tuple[int, int, str]:
    ...
    value = (
        int(payload.get("model_step_seconds", -1)),
        int(payload.get("horizon_steps", -1)),
        str(payload.get("swmm_engine_version", "")).strip(),
    )
...
def _hardware(device: torch.device) -> dict[str, object]:
    ...
    try:
        ...
        payload.update(
            {
                "system_ram_total_gb": float(vm.total / 1024**3),
                "system_ram_available_gb_at_start": float(vm.available / 1024**3),
            }
        )
    except Exception:
        ...
        payload.update(
            {
                ...
                "cuda_free_gb_at_start": float(free / 1024**3),
                "cuda_allocator_backend": str(torch.cuda.get_allocator_backend()),
            }
        )
        if total < 7.0 * 1024**3:
...
def _parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--stop-after-stage",
        choices=("stage_a", "stage_b0", "objective", "evaluation"),
        default="evaluation",
    )
    p.add_argument(
        "--profile-one-group",
        action="store_true",
        help="Diagnostic-only: one deterministic group per source; forbidden with full.",
    )
    p.add_argument(
        "--torch-profiler",
        action="store_true",
        help="Export CPU/CUDA trace; requires smoke/dev plus --profile-one-group.",
    )
    return p
...
def _target_reached(completed: str, target: str) -> bool:
    return _STAGE_ORDER[completed] >= _STAGE_ORDER[target]
...
def main() -> None:
    ...
    if args.torch_profiler and not args.profile_one_group:
        raise ValueError("--torch-profiler requires --profile-one-group to bound profiling cost")
    ...
    telemetry.emit(
        ...
        one_group=bool(args.profile_one_group),
    ...
    if args.resume_from:
        payload = load_stage_checkpoint_v128(
            ...
            expected_training_design=design_dict,
        )
        ...
        telemetry.emit(
            ...
            path=str(Path(args.resume_from).resolve()),
        )
    ...
    target_stage = str(args.stop_after_stage)
    with profiler:
        if _should_run(completed, "stage_a") and not _target_reached(completed, target_stage):
            with TimedStage(telemetry, "stage_a"):
                history["hydraulic"] = train_hydraulic_stage_streaming_v128(
                    model,
            ...
            save_stage_checkpoint_v128(
                ...
                graph_path=args.graph,
                lineage=lineage,
                training_design=design_dict,
                history=history,
            )

        if _should_run(completed, "stage_b0") and not _target_reached(completed, target_stage):
            with TimedStage(telemetry, "stage_b0"):
                history["rollout"] = train_truncated_rollout_stage_streaming_v127(
                    model,
                    source_caches=source_caches,
                    source_groups=source_groups,
                    normalization=normalization,
        ...
        if _should_run(completed, "objective") and not _target_reached(completed, target_stage):
            with TimedStage(telemetry, "objective"):
                history["objective"] = train_objective_stage_streaming_v128(
                    model,
                    source_caches=source_caches,
                    source_groups=source_groups,
                    normalization=normalization,
    ...
    if target_stage != "evaluation" and _target_reached(completed, target_stage):
        telemetry.emit("run_paused", profile=profile.name, completed_stage=completed)
        pause = {
            "contract": CURRENT_RUN_CONTRACT,
            "profile": profile.name,
            "completed_stage": completed,
            "resume_checkpoint": str((out / {"stage_a": "stage_a.pt", "stage_b0": "stage_b0.pt", "objective": "stage_objective.pt"}[completed]).resolve()),
            "scientific_claim_allowed": False,
            "final_checkpoint_created": False,
        }
        (out / "PAUSED_RUN.json").write_text(
            json.dumps(pause, indent=2, sort_keys=True, allow_nan=False) + "\n",
            encoding="utf-8",
        )
        print(json.dumps(pause, indent=2, sort_keys=True, allow_nan=False))
        return
    ...
    specs = {
        ...
        "d4_fit": (d4_fit_online, selected["d4_fit"]),
        "d4_audit": (d4_audit_online, selected["d4_audit"]),
    }
    evaluations: dict[str, dict[str, float | str]] = {}
    with TimedStage(telemetry, "evaluation"):
        for key, (cache, names) in specs.items():
            metrics = evaluate_objective_groups_streaming_v127(
                model,
                cache=cache,
                names=names,
                normalization=normalization,
    ...
    report = {
        ...
        "hydraulic_training_contract": V128_HYDRAULIC_TRAINING_CONTRACT,
        "objective_training_contract": V128_OBJECTIVE_TRAINING_CONTRACT,
        "control_profile_contract": V128_CONTROL_PROFILE_CONTRACT,
        "rtc_implementation_contract_sha256": rtc_implementation_contract_sha256(),
        "hardware": {**hardware, **execution_profile},
        "memory_execution": asdict(memory),
        "training_design": design_dict,
        "selected_groups": selected,
        "selected_group_counts": {key: len(value) for key, value in selected.items()},
        "residual_scale_sampling": scale_telemetry,
        "history": history,
    ...
    if profile.final_checkpoint_allowed:
        checkpoint = save_step2_v128(
            out / FINAL_CHECKPOINT_FILENAME,
            model=model,
            graph=graph,
    ...
    telemetry.emit(
        "run_complete",
        profile=profile.name,
        strict_final_checkpoint=bool(profile.final_checkpoint_allowed),
    )
