"""Fine-tune V127 Step2 with D5-FIT gradients using CPU-staged causal inputs only."""
...
from rtc.step2_gradient_v127_streaming import (
...
V127_D5_STREAMING_RUN_CONTRACT = (
    "PROJECT7_V127_D5_GRADIENT_FINETUNE_STREAMING_V1_16GB_RAM_8GB_VRAM"
)
...
def main() -> None:
    ...
    if device.type == "cuda":
        ...
        if total < 7.0 * 1024**3:
            raise RuntimeError("V127 D5 streaming profile expects an approximately 8-GB CUDA GPU")
        if free < 4.0 * 1024**3:
    ...
    before_detail, before = evaluate_d5_gradients_streaming_v127(
        model,
        cache=online,
        cases=audit_cases,
    ...
    history = train_d5_gradient_streaming_v127(
        model,
        cache=online,
        cases=fit_cases,
        normalization=normalization,
    ...
    after_detail, after = evaluate_d5_gradients_streaming_v127(
        model,
        cache=online,
        cases=audit_cases,
        normalization=normalization,
    ...
    for key in (
        ...
        lineage[key] = value
    if lineage["causal_state_step1_model_semantic_sha256"] != str(
        state.step1_model_semantic_sha256
    ):
        raise ValueError("V127 D5 Step1 model semantics differ from the base Step2 checkpoint")
    if lineage["causal_state_sensor_layout_semantic_sha256"] != str(
        state.sensor_layout_semantic_sha256
    ):
        raise ValueError("V127 D5 sensor layout semantics differ from the base Step2 checkpoint")

    report = {
        "contract": V127_D5_STREAMING_RUN_CONTRACT,
