"""Run refined-first-move Direct-TFV V9 in an authoritative Development SWMM closed loop."""
...
from rtc.step3_tfv_value_mpc_v9 import DirectTFVRecedingMPCV9


DIRECT_TFV_DEVELOPMENT_RUNTIME_CONTRACT = (
    "PROJECT7_DIRECT_TFV_AUTHORITATIVE_10MIN_DEVELOPMENT_RTC_V8"
)
...
def _load_json(path: str | Path, *, contract: str, label: str) -> dict:
    ...
    if not isinstance(payload, dict) or str(payload.get("contract", "")) != contract:
        raise ValueError(f"runtime requires current {label} artifact")
...
def main() -> None:
    ...
    p.add_argument("--first-move-admission-calibration", required=True)
    ...
    if not 0.0 < float(args.optimizer_deadline_seconds) < float(args.decision_runtime_budget_seconds) < 600.0:
        raise ValueError("Direct-TFV runtime requires optimizer deadline < controller budget < 600 s")
    if float(args.optimizer_deadline_seconds) + float(args.first_move_deadline_seconds) >= float(args.decision_runtime_budget_seconds):
        raise ValueError("full-plan plus first-move solver budgets must fit inside controller runtime budget")
    ...
    first = _load_json(
        args.first_move_admission_calibration,
        contract=DIRECT_TFV_FIRST_MOVE_ADMISSION_CONTRACT,
        label="refined first-move admission",
    )
    ...
    if not isinstance(lineage, dict) or str(lineage.get("step2_checkpoint_sha256", "")).lower() != _sha(args.step2).lower():
        raise ValueError("first-move admission was derived from a different Step2 checkpoint")
    current_source_sha = direct_tfv_first_move_source_sha256()
    if str(lineage.get("first_move_source_sha256", "")).lower() != current_source_sha.lower():
        raise ValueError(
            "first-move admission was generated by a different Direct-TFV first-move source; "
            "regenerate the first-move panel and calibration before authoritative runtime"
        )
    ...
    if str(lineage.get("sequence_support_sha256", "")).lower() != _sha(args.sequence_support).lower():
        raise ValueError("first-move admission was derived from a different sequence-support artifact")
    ...
    if write_audit.get("passed") is not True:
        raise RuntimeError("Direct-TFV V9 authoritative run failed target write/readback audit")
    ...
    metadata.update(
        {
            ...
            "refined_first_move_execution_used": True,
            ...
            "first_move_calibration_source_sha256": str(lineage["first_move_source_sha256"]),
            "runtime_first_move_source_sha256": current_source_sha,
