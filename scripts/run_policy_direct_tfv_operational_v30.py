...
def main() -> None:
    ...
    controller, _, sensors, lineage = build_operational_v30_controller(
    ...
    if write_audit.get("passed") is not True:
        raise RuntimeError("operational V30 failed target write/readback audit")
    ...
    metadata.update(
        {
            ...
            "same_sparse_sensor_information_as_matched_baselines": True,
            "online_swmm_candidate_search": False,
            "future_realized_rainfall_used_as_model_input": False,
            "projected_gradient_h10_enabled": False,
            # These hashes are provenance only; V30 adds no new hash-based action/science gate.
