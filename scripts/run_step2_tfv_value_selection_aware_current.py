"""Current Project7 Step2 runner with selection-aware Direct-TFV training.

The model architecture remains the validated pairwise V(candidate)-V(reference) Direct-TFV V2.
This runner changes the *training objective*, not the research target: after MAIN and JOINT stages it
performs a low-learning-rate selection stage that explicitly learns the HOLD=0 decision boundary.
It also derives TrainFit-only 109-facility coverage and action-support statistics for the Step3 trust
region.

No SWMM is launched here.  D4 AUDIT remains evaluation-only and never enters fitting, target-scale
fitting, action-support derivation, or selection-loss training.
"""
...
from rtc.step2_tfv_value_training_v3 import (
...
CURRENT_DIRECT_TFV_RUN_CONTRACT = "PROJECT7_CURRENT_DIRECT_TFV_SELECTION_AWARE_SMOKE_DEV_V3"
...
def _parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--selection-epochs", type=int, default=0)
    p.add_argument("--learning-rate", type=float, default=5.0e-4)
    p.add_argument("--selection-learning-rate", type=float, default=1.0e-4)
    return p


def _training_design(profile: str, args: argparse.Namespace) -> DirectTFVTrainingDesignV3:
    if profile == "smoke":
        main_epochs, interaction_epochs, selection_epochs = 2, 1, 1
    else:
        main_epochs, interaction_epochs, selection_epochs = 8, 5, 3
    ...
    if int(args.selection_epochs) > 0:
        selection_epochs = int(args.selection_epochs)
    design = DirectTFVTrainingDesignV3(
        ...
        selection_learning_rate=float(args.selection_learning_rate),
        seed=SEED,
    )
    design.validate()
    return design
...
def main() -> None:
    ...
    if len(graph.actuator_ids) != 109:
        raise ValueError("selection-aware Direct-TFV requires exactly 109 actuators")
    ...
    if profile_name == "dev" and int(action_support["single_facility_coverage_count"]) != 109:
        raise RuntimeError(
            "full Direct-TFV DEV cannot claim 109-facility learning without 109/109 exact single-facility TrainFit coverage: "
    ...
    history = train_direct_tfv_value_model_v3(
        model,
    ...
    report = {
        ...
        "selection_aware_training": True,
        "selection_stage_role": "explicit HOLD/action sign and oracle-choice calibration without changing the primary delta-TFV target",
        ...
        "next_gate": "HOLD-reference selection threshold calibration and trust-region Step3 solver-only Development audit",
    }
