...
The rule sees reconstructed hydraulic state only. It does not receive extra node depths from SWMM.
...
- native `internal_rtc`: external operational reference only. It directly evaluates SWMM internal
  rule conditions and therefore cannot honestly be called information-matched without a separately
  validated reconstructed-state SWMM-controls interpreter. This branch refuses to relabel it.
...
Only after those gates pass may the frozen Benchmark5 events run `matched_auto_rbc` and `matched_efd`.
Those new matched-baseline runs must use fresh output directories and must never overwrite the native
baseline cache.
