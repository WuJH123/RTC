"""Development-only operational V23 runtime for strong-storm TFV steering.

V23 keeps the frozen Step1, base Step2, V15 rank branch, V21 selected/HOLD boundary, q95 support,
three-family portfolio cardinality and 0.5 command slew.  It changes only the target generated by the
existing TYPE_AWARE_HYDRAULIC_PRESSURE family under broadly-high causal filling, via
``build_hybrid_policy_return_portfolio_v23``.

Because the candidate distribution no longer exactly matches V21 training, this lane is expressly
non-promotable Development evidence until a matched candidate bank/boundary calibration is rebuilt.
"""
...
from .direct_tfv_operational_v21_runtime_v2 import (
...
from .direct_tfv_policy_return_query_margin_v17 import rank_state_sha256
...
class DirectTFVOperationalV23MPC(DirectTFVOperationalV21MPCV2):
    policy_mode = "operational_development_v23_strongstorm_hydraulic_candidate"
    policy_mode_contract = OPERATIONAL_V23_RUNTIME_CONTRACT
    ...
    def optimize(self, **kwargs: Any) -> DirectTFVMPCResultV12:
        ...
        if not isinstance(active_target, torch.Tensor) or tuple(active_target.shape) != (109,):
            raise ValueError("operational V23 requires active_target [109]")
        current_state = kwargs["current_state"]
        rainfall = kwargs["rainfall"]
        flow = kwargs["previous_actuator_flow"]
        if tuple(flow.shape) != (1, 109):
            raise ValueError("operational V23 requires previous_actuator_flow [1,109]")
        ...
        for proposal in hybrid.candidates:
            ...
            if bool(torch.any(torch.abs(target[passive] - active_target[passive]) > 1.0e-7)):
                raise RuntimeError("operational V23 candidate changed a passive channel")
        ...
        return DirectTFVMPCResultV12(
            ...
            selected_source=(
                f"OPERATIONAL_V23::{row['source']}"
                if passed
                else "LATCH_PREVIOUS_TARGET_OPERATIONAL_V23_BOUNDARY_NONNEGATIVE"
            ),
            candidate_valid=passed,
            admission_margin_m3=0.0,
            admission_upper_bound_m3=advantage,
            admission_margin_kind=(
                "development_raw_v21_zero_boundary_on_v23_candidate_distribution"
            ),
            ...
            gradient_norm=0.0,
            scipy_message=(
                "NOT_USED_OPERATIONAL_V23_FINITE_PORTFOLIO|"
...
def build_operational_v23_controller(
    *,
    graph_path: str | Path,
    ...
    if str(boundary_checkpoint.get("base_step2_sha256", "")).lower() != step2_sha.lower():
        raise ValueError("V23 boundary/base Step2 lineage mismatch")
    ...
    if str(boundary_checkpoint.get("rank_source_checkpoint_sha256", "")).lower() != rank_source_sha.lower():
        raise ValueError("V23 boundary/V15 rank lineage mismatch")
    if str(boundary_checkpoint.get("supervisory_mask_sha256", "")).lower() != str(
        control["supervisory_mask_sha256"]
    ).lower():
        raise ValueError("V23 boundary uses another supervisory mask")
    ...
    if len(expected_rank_sha) != 64 or imported_rank_sha.lower() != expected_rank_sha:
        raise ValueError("V23 runtime V15 rank does not reproduce V21 training rank")
    parent = str(boundary_checkpoint.get("continuation_policy_sha256", "")).lower()
    if len(parent) != 64:
        raise ValueError("V23 boundary lacks continuation-policy lineage")
    ...
    controller_cfg = replace(
        ...
        fallback_policy_id="HOLD_OPERATIONAL_V23_DEVELOPMENT_FALLBACK",
    )
    ...
    mpc = DirectTFVOperationalV23MPC(
        model=base_model,
        graph=graph,
    ...
    lineage = {
        "contract": OPERATIONAL_V23_RUNTIME_CONTRACT,
        ...
        "boundary_is_distribution_matched_to_v23_candidates": False,
