...
def accept_step2_large_v2_main() -> None:
    parser = argparse.ArgumentParser(
        description="Accept Step2 on engine-matched exact SWMM truth with group-balanced metrics"
    )
    ...
    if str(model_meta.get("swmm_engine_version", "")) != str(manifest["swmm_engine_version"]):
        raise ValueError("Step2 validation SWMM engine differs from training checkpoint")
        ...
        for item in manifest["shards"]:
            with np.load(str(item["path"]), allow_pickle=False) as ds:
                if "exact_node_flood_volume_m3" not in ds.files:
                    raise ValueError("Step2 Formal acceptance requires exact SWMM node flooding-volume truth")
                ...
                if str(ds["swmm_engine_version"].item()) != str(manifest["swmm_engine_version"]):
                    raise ValueError("Step2 validation shard embedded engine mismatch")
                ...
                for start in range(0, count, args.batch_size):
                    ...
                    rollout = model.rollout(
                        ...
                        physics.unsqueeze(0).expand(b, -1, -1), static, edges,
                    )
                    ...
                    pred_node = trapezoid_node_flood_volume(
                        initial, rollout.states, flood_rate_index=2,
                    ...
                    for local, group_id in enumerate(rainfall_groups[start:end]):
                        ...
                        agg["depth_sse"] = float(agg["depth_sse"]) + float(np.square(depth_err).sum())
                        agg["depth_n"] = int(agg["depth_n"]) + int(depth_err.size)
                        agg["flow_sse"] = float(agg["flow_sse"]) + float(np.square(flow_err).sum())
                        ...
                        agg["negative_depth_count"] = int(agg["negative_depth_count"]) + int(np.count_nonzero(np.isfinite(depth) & (depth < negative_tolerance)))
                        agg["depth_value_count"] = int(agg["depth_value_count"]) + int(depth.size)
                        agg["negative_flood_rate_count"] = int(agg["negative_flood_rate_count"]) + int(np.count_nonzero(np.isfinite(flood_rate) & (flood_rate < negative_tolerance)))
                        agg["flood_rate_value_count"] = int(agg["flood_rate_value_count"]) + int(flood_rate.size)
                        agg["negative_node_volume_count"] = int(agg["negative_node_volume_count"]) + int(np.count_nonzero(np.isfinite(node_volume) & (node_volume < negative_tolerance)))
                        agg["node_volume_value_count"] = int(agg["node_volume_value_count"]) + int(node_volume.size)
                        agg["nonfinite_state_count"] = int(agg["nonfinite_state_count"]) + int(state_values.size - np.isfinite(state_values).sum())
                        agg["state_value_count"] = int(agg["state_value_count"]) + int(state_values.size)
                        agg["nonfinite_flow_count"] = int(agg["nonfinite_flow_count"]) + int(flow_values.size - np.isfinite(flow_values).sum())
                        agg["flow_value_count"] = int(agg["flow_value_count"]) + int(flow_values.size)
                        if pidx is not None:
    ...
    for group_id, agg in sorted(groups.items()):
        ...
        row: dict[str, float | str] = {
            ...
            "depth_rmse_m": float(np.sqrt(float(agg["depth_sse"]) / max(int(agg["depth_n"]), 1))),
            "managed_flow_rmse_m3s": float(np.sqrt(float(agg["flow_sse"]) / max(int(agg["flow_n"]), 1))),
            "tfv_exact_truth_mae_m3": float(np.mean(np.abs(pred_tfv - true_tfv))),
            "tfv_exact_truth_rank_correlation": float(rank_correlation(pred_tfv, true_tfv)),
            "negative_depth_fraction": _fraction(agg["negative_depth_count"], agg["depth_value_count"]),
            "negative_flooding_rate_fraction": _fraction(agg["negative_flood_rate_count"], agg["flood_rate_value_count"]),
            "negative_node_volume_fraction": _fraction(agg["negative_node_volume_count"], agg["node_volume_value_count"]),
            "nonfinite_state_fraction": _fraction(agg["nonfinite_state_count"], agg["state_value_count"]),
            "nonfinite_actuator_flow_fraction": _fraction(agg["nonfinite_flow_count"], agg["flow_value_count"]),
        }
        if pidx is not None:
            pred_pfv = np.asarray(agg["pred_pfv"], dtype=float)
            true_pfv = np.asarray(agg["true_pfv"], dtype=float)
            row["priority_flood_exact_truth_mae_m3"] = float(np.mean(np.abs(pred_pfv - true_pfv)))
            row["priority_flood_exact_truth_rank_correlation"] = float(rank_correlation(pred_pfv, true_pfv))
        group_metrics.append(row)

    metric_names = [
        "depth_rmse_m", "managed_flow_rmse_m3s", "tfv_exact_truth_mae_m3",
        "tfv_exact_truth_rank_correlation", "negative_depth_fraction",
        "negative_flooding_rate_fraction", "negative_node_volume_fraction",
        "nonfinite_state_fraction", "nonfinite_actuator_flow_fraction",
    ]
    if pidx is not None:
        metric_names += ["priority_flood_exact_truth_mae_m3", "priority_flood_exact_truth_rank_correlation"]
    metrics = {name: float(np.mean([float(row[name]) for row in group_metrics])) for name in metric_names}
    payload = {
        ...
        "physical_plausibility_diagnostics": {
            ...
            "interpretation": "Basic physical plausibility diagnostics; authoritative hydraulic/flood truth remains SWMM.",
        },
