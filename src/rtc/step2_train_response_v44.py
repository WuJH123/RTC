"""Bounded V4.4 edge-residual-only development training."""
...
from .step2_edge_hydraulic_v44 import PhysicalDirectedEdgeLineageV44, normalize_edge_static_features_v44
...
from .step2_control_response_v44 import (
    DifferentiableCounterfactualResponseModelV44,
    set_trainable_edge_hydraulic_v44,
)


TRAINING_CONTRACT_V44 = "PROJECT7_STEP2_EDGE_HYDRAULIC_INTERACTION_V44"
...
def train_edge_hydraulic_v44(
    *,
    ...
    """Train only V4.4 edge parameters with the frozen V4.3.3 objective."""

    if not grouped_pairs:
        raise ValueError("edge residual training requires at least one group")
    ...
    names = set_trainable_edge_hydraulic_v44(model, enabled=True)
    parameters = [parameter for parameter in model.parameters() if parameter.requires_grad]
    if not parameters:
        raise RuntimeError("V4.4 edge phase selected no parameters")
    ...
    for epoch in range(1, int(epochs) + 1):
        ...
        for group in order:
            ...
            if not torch.isfinite(loss):
                raise FloatingPointError(f"non-finite V4.4 loss at epoch {epoch}, group {group}")
            ...
            if not np.isfinite(gradient):
                raise FloatingPointError(f"non-finite V4.4 gradient at epoch {epoch}, group {group}")
            tick = time.perf_counter()
            optimizer.step()
    ...
    return {
        "contract": TRAINING_CONTRACT_V44,
        ...
        "edge_parameter_sha256": parameter_sha256(model, names),
    }


__all__ = ["TRAINING_CONTRACT_V44", "train_edge_hydraulic_v44"]
