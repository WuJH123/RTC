...
def _actuator_flow(metadata_path: str | Path, actuator_id: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    frame = pd.read_csv(p.parent / str(meta["actuator_file"]), compression="infer")
    ...
    flow = flow_rate_to_m3s(frame["flow"].to_numpy(dtype=float), str(meta["flow_units"]))
    return time, flow
...
def build_timescale_report(
    *, manifest_path: str | Path, run_summary_path: str | Path, split: str = "development"
        ...
        for _, candidate in group.iterrows():
            ...
            if peak <= 1e-12:
                ...
                if len(t) > 1:
                    ...
                    if total > 0:
                        mass_idx = int(np.searchsorted(area, 0.90 * total, side="left")) + 1
                        mass_idx = min(mass_idx, len(rel_t) - 1)
    ...
    summary: dict[str, object] = {
        "contract": "PHASE0_D2_HYDRAULIC_TIMESCALE_REPORT_V2",
        "split": split,
        ...
        "instruction": "Freeze model_step_seconds, control_update_seconds and horizon_steps only after development review of these distributions plus closed-loop sensitivity. Do not inherit old project values by default.",
    }
    return detail, summary


def main() -> None:
    parser = argparse.ArgumentParser(description="Report empirical D2 hydraulic response time scales")
