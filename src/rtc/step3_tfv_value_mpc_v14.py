...
class DirectTFVQueryMarginMPCV14(DirectTFVHybridPolicyReturnMPCV13):
    ...
    def __init__(
        self,
        *,
        query_margin_adapter: QueryConditionedPolicyReturnAdapter,
        **kwargs: Any,
    ) -> None:
    ...
    def optimize(self, **kwargs: Any) -> DirectTFVMPCResultV12:
        ...
        if not isinstance(active_target, torch.Tensor) or tuple(active_target.shape) != (
            109,
        ):
        ...
        if tuple(flow.shape) != (1, 109):
            raise ValueError("query-conditioned Step3 requires flow [1,109]")
        ...
        for proposal in hybrid.candidates:
            ...
            if changed <= 0:
                continue
            if bool(
                torch.any(torch.abs(target[passive] - active_target[passive]) > 1e-7)
            ):
        ...
        raw_rank = torch.as_tensor(
            [row[4] for row in evaluated],
            dtype=targets.dtype,
            device=targets.device,
        )
        base_scores = torch.as_tensor(
            [row[5] for row in evaluated],
            dtype=targets.dtype,
            device=targets.device,
        )
        context_features, candidate_features = build_query_margin_features(
            ...
            candidate_sources=[str(row[0]) for row in evaluated],
            supervisory_mask=self.supervisory_mask,
        ...
        source, target, sequence, changed, raw_score, base_score, diagnostics = evaluated[
            selected_index
        ]
        score = float(predicted[selected_index])
        query_margin = float(joint.query_best_margin_m3.detach().cpu())
        if abs(score - query_margin) > max(
            1e-4,
            1e-6 * max(1.0, abs(query_margin)),
        ):
            raise RuntimeError(
                "selected query-margin candidate is not anchored at the query best margin"
            )
        ...
        hold = (
            active_target.reshape(1, 109)
            .expand(int(self.design.prediction_horizon_steps), -1)
            .detach()
        )
        executed = sequence if passed else hold
        changed_indices = (
            torch.nonzero(torch.abs(target - active_target) > 1e-7).reshape(-1).tolist()
        )
        changed_ids = tuple(
        ...
        for row, pred in zip(evaluated, predicted.tolist()):
            uppers.append(
                float(pred)
                + float(policy_return_margin_m3(self.policy_return_admission, int(row[3])))
            )
        ...
        return DirectTFVMPCResultV12(
            ...
            selected_source=(
                f"DIRECT_TFV_QUERY_MARGIN::{source}"
                if passed
                else "LATCH_PREVIOUS_TARGET_QUERY_MARGIN_UCB_NONNEGATIVE"
            ),
            ...
            gradient_norm=0.0,
            scipy_message=(
                "NOT_USED_QUERY_CONDITIONED_THREE_FAMILY_FINITE_PORTFOLIO::"
                + DIRECT_TFV_QUERY_MARGIN_CONTRACT
            ),
