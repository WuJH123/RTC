...
from .step3_tfv_value_mpc_v14 import DIRECT_TFV_QUERY_MARGIN_STEP3_CONTRACT, DirectTFVQueryMarginMPCV14
...
def build_frozen_query_margin_controller(
    *,graph_path:str|Path,sensors_path:str|Path,config_path:str|Path,step1_path:str|Path,
    step2_path:str|Path,supervisory_control_path:str|Path,sequence_support_path:str|Path,
    policy_return_checkpoint_path:str|Path,policy_return_admission_path:str|Path,
    device:torch.device,decision_runtime_budget_seconds:float=180.0,proposal_probe_chunk_size:int=24,
    projected_gradient_steps:int=6,projected_gradient_step_fraction:float=0.25,
)->tuple[object,object,tuple[str,...],dict]:
    graph=_load_graph(graph_path); sensors=_load_lines(sensors_path)
    ...
    validate_direct_tfv_sequence_support(
        ...
        supervisory_mask=mask,supervisory_control_contract=str(control["contract"]),
    )
    rank_model,rank_norm,adapter,checkpoint=load_query_margin_checkpoint(
        policy_return_checkpoint_path,graph=graph,base_step2_path=step2_path,device=device
    )
    if checkpoint.get("fresh_validation_verified") is not True:
        raise ValueError("query-margin runtime requires a critic accepted on fresh validation")
    admission=json.loads(Path(policy_return_admission_path).read_text(encoding="utf-8"))
    ...
    if str(admission.get("policy_return_checkpoint_sha256","")).lower()!=checkpoint_sha.lower():
    ...
    if len(parent)!=64 or parent!=str(admission.get("continuation_policy_sha256","")).lower():
    ...
    mpc=DirectTFVQueryMarginMPCV14(
        ...
        policy_return_normalization=rank_norm,query_margin_adapter=adapter,
        policy_return_admission=admission,policy_return_checkpoint_sha256=checkpoint_sha,
        design=design,proposal_probe_chunk_size=int(proposal_probe_chunk_size),
    ...
    lineage={
        ...
        "step1_sha256":_sha(step1_path),"base_step2_sha256":_sha(step2_path),
        "supervisory_control_sha256":_sha(supervisory_control_path),
        "supervisory_control_contract":control["contract"],"supervisory_control_dimension":int(mask.sum()),
        "model_action_channel_count":109,"sequence_support_sha256":_sha(sequence_support_path),
        ...
        "candidate_portfolio_families":["STEP2_H10_PROBE_SCALE_0.50","STEP2_H10_PROBE_SCALE_1.00","TYPE_AWARE_HYDRAULIC_PRESSURE"],
        ...
        "projected_gradient_h10_enabled":False,"projected_gradient_ablation_available":True,
        "projected_gradient_generator_contract":DIRECT_TFV_PROJECTED_GRADIENT_GENERATOR_CONTRACT,
        "projected_gradient_cli_knobs_affect_current_policy":False,"portfolio_mode":True,
        "online_lbfgsb_used":False,"h10_probe_generator_contract":DIRECT_TFV_H10_PROBE_GENERATOR_CONTRACT,
        "legacy_v12_admission_required_online":False,"graph_sha256":_sha(graph_path),
        "sensors_sha256":_sha(sensors_path),"config_sha256":_sha(config_path),
...
__all__=["QUERY_MARGIN_RUNTIME_FACTORY_CONTRACT","build_frozen_query_margin_controller"]
