...
def verify_formal_run_v4(
    ...
    *,
    ...
    if run.get("contract") != "FORMAL_CLOSED_LOOP_RUN_MANIFEST_V4_CODE_BOUND":
        raise ValueError(f"not a code-bound Formal run V4 manifest: {manifest_path}")
    if run.get("rtc_source_tree_sha256") != current_code:
        raise ValueError(f"Formal run was produced by a different RTC source tree: {manifest_path}")
    ...
    if meta.get("rtc_source_tree_sha256") is not None and meta.get(
        ...
        raise ValueError("main policy run metadata comes from a different RTC source tree")
    ...
    if replay.get("contract") != "ROUTING_STEP_GLOBAL_PEAK_REPLAY_V2_ISOLATED":
        raise ValueError("formal run lacks isolated routing-step Global Peak replay V2")
    if replay.get("rtc_source_tree_sha256") != current_code:
        raise ValueError("Global Peak replay was generated by a different RTC source tree")
