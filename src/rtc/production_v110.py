"""Development production-path wiring for Step1 -> V7/V11 MPC -> first move.

This module deliberately reuses the existing authoritative closed-loop runner and
``TorchMPCController``. The only new controller component is the fail-closed
V7/V11 surrogate stack. Formal promotion remains a separate gate.
"""
from __future__ import annotations

import argparse
import json
from pathlib import Path

import torch

from .closed_loop import run_authoritative_closed_loop
from .controller import TorchMPCController
from .forecast import PersistenceDecayForecast
from .production_cli import (
    _controller_config,
    _controls_disabled_runtime,
    _load_graph,
    _load_lines,
    _load_step1,
    _priority_and_calibration,
)
from .runtime_controller_guard import ContinuityGuardController
from .step2_runtime_v110 import load_v7_v11_rolling_mpc


def _v110_bundle_metadata(path: str | Path) -> dict[str, object]:
    payload = torch.load(path, map_location="cpu", weights_only=False)
    if not isinstance(payload, dict):
        raise ValueError("V11 runtime bundle must be a dictionary")
    required = (
        "runtime_bundle_contract",
        "runtime_compatible",
        "production_compatible",
        "swmm_engine_version",
        "model_step_seconds",
        "value_horizon_steps",
        "hydraulic_horizon_minutes",
        "control_update_seconds",
    )
    missing = [name for name in required if name not in payload]
    if missing:
        raise ValueError(f"V11 runtime bundle missing metadata: {missing}")
    return {name: payload[name] for name in required}


def run_policy_v110_main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Project7 V7 Value + V11 Hydraulic rolling MPC development path"
    )
    parser.add_argument("--strategy", required=True, choices=("proposed",))
    parser.add_argument("--inp", required=True)
    parser.add_argument("--out-dir", required=True)
    parser.add_argument("--run-id", required=True)
    parser.add_argument("--sensors", required=True)
    parser.add_argument("--priority")
    parser.add_argument("--config", required=True)
    parser.add_argument("--graph", required=True)
    parser.add_argument("--step1", required=True)
    parser.add_argument("--step2-value", required=True)
    parser.add_argument("--step2-reference", required=True)
    parser.add_argument("--step2-v110", required=True)
    parser.add_argument("--runtime-inp-cache-dir")
    parser.add_argument("--device")
    parser.add_argument(
        "--allow-development-v110",
        action="store_true",
        help="allow a runtime-compatible but not production-promoted V11 bundle for bounded development smoke",
    )
    args = parser.parse_args()

    cfg = json.loads(Path(args.config).read_text(encoding="utf-8"))
    if not isinstance(cfg, dict):
        raise ValueError("controller config must be a JSON object")
    model_step_seconds = int(cfg["model_step_seconds"])
    control_update_seconds = int(cfg["control_update_seconds"])
    record_stride_seconds = int(cfg.get("record_stride_seconds", model_step_seconds))
    if model_step_seconds != 300:
        raise ValueError("V11 production path requires frozen 300-s model step")
    if control_update_seconds != 600:
        raise ValueError("V11 production path requires frozen 600-s decision update")
    if record_stride_seconds != model_step_seconds:
        raise ValueError("V11 production record stride must equal the model step")
    control_block_steps = control_update_seconds // model_step_seconds
    control_start_minutes = int(cfg.get("control_start_minutes", 0))
    controller_cfg_raw = cfg.get("controller", {})
    if not isinstance(controller_cfg_raw, dict):
        raise ValueError("config controller must be an object")
    if int(controller_cfg_raw.get("horizon_steps", -1)) != 72:
        raise ValueError("V11 production controller requires the V7 360-min/72-step value horizon")
    raw_delta = controller_cfg_raw.get("max_setting_delta_per_update")
    if raw_delta is None or float(raw_delta) > 0.5 + 1.0e-9:
        raise ValueError("V11 production path requires max setting delta <= frozen 0.5/update")

    device = torch.device(args.device or ("cuda" if torch.cuda.is_available() else "cpu"))
    graph = _load_graph(args.graph)
    sensors = _load_lines(args.sensors)
    if not set(sensors).issubset(graph.node_ids):
        raise ValueError("V11 runtime sensor nodes are not all present in graph schema")
    step1 = _load_step1(args.step1, device)
    step1_meta = dict(getattr(step1, "runtime_metadata", {}))
    if int(step1_meta.get("model_step_seconds", -1)) != model_step_seconds:
        raise ValueError("Step1 model step differs from V11 production controller")
    if int(step1_meta.get("history_steps", -1)) != int(controller_cfg_raw.get("history_steps", -1)):
        raise ValueError("Step1 history length differs from V11 production controller")

    bundle_meta = _v110_bundle_metadata(args.step2_v110)
    if int(bundle_meta["model_step_seconds"]) != model_step_seconds:
        raise ValueError("V11 bundle model step differs from controller")
    if int(bundle_meta["control_update_seconds"]) != control_update_seconds:
        raise ValueError("V11 bundle decision interval differs from controller")
    if int(bundle_meta["value_horizon_steps"]) != 72:
        raise ValueError("V11 bundle value horizon differs from frozen V7 horizon")
    if int(bundle_meta["hydraulic_horizon_minutes"]) != 120:
        raise ValueError("V11 bundle hydraulic horizon differs from frozen 120 min")
    step1_engine = str(step1_meta.get("swmm_engine_version", "")).strip()
    bundle_engine = str(bundle_meta["swmm_engine_version"]).strip()
    if not step1_engine or step1_engine != bundle_engine:
        raise ValueError(
            f"Step1/V11 SWMM engine lineage differs: {step1_engine} != {bundle_engine}"
        )

    priority_indices, _, _ = _priority_and_calibration(
        priority_path=args.priority,
        calibration_path=None,
        graph=graph,
        device=device,
    )
    objective_cfg = cfg.get("objective", {})
    if not isinstance(objective_cfg, dict):
        raise ValueError("config objective must be an object")
    mpc = load_v7_v11_rolling_mpc(
        graph=graph,
        v70_value_checkpoint=args.step2_value,
        v70_hydraulic_checkpoint=args.step2_reference,
        v110_hydraulic_checkpoint=args.step2_v110,
        device=device,
        priority_indices=priority_indices,
        allow_development=bool(args.allow_development_v110),
        tfv_cvar_alpha=float(objective_cfg.get("tfv_cvar_alpha", 0.90)),
        tfv_near_opt_relative=float(objective_cfg.get("tfv_near_opt_relative", 0.01)),
        tfv_near_opt_absolute_m3=float(objective_cfg.get("tfv_near_opt_absolute_m3", 1.0)),
        near_opt_penalty=float(objective_cfg.get("near_opt_penalty", 1.0e4)),
        movement_tiebreak=float(objective_cfg.get("movement_tiebreak", 1.0e-6)),
        min_predicted_tfv_improvement_m3=float(
            objective_cfg.get("min_predicted_tfv_improvement_m3", 0.0)
        ),
        hydraulic_secondary_iterations=int(
            objective_cfg.get("v110_hydraulic_secondary_iterations", 8)
        ),
    )

    forecast_cfg = cfg.get("forecast", {})
    if not isinstance(forecast_cfg, dict):
        raise ValueError("config forecast must be an object")
    forecast = PersistenceDecayForecast(
        decay_per_step=float(forecast_cfg.get("decay_per_step", 0.92)),
        scenario_multipliers=tuple(
            float(x) for x in forecast_cfg.get("scenario_multipliers", [0.75, 1.0, 1.25])
        ),
        history_steps_for_level=int(forecast_cfg.get("history_steps_for_level", 3)),
    )
    controller = TorchMPCController(
        step1=step1,
        mpc=mpc,
        graph=graph,
        sensor_nodes=sensors,
        forecast=forecast,
        config=_controller_config(controller_cfg_raw, control_block_steps=control_block_steps),
        device=device,
    )
    controller = ContinuityGuardController(
        controller,
        max_delta_per_update=float(raw_delta),
        allow_projection=False,
    )

    source_inp = Path(args.inp)
    cache_dir = (
        Path(args.runtime_inp_cache_dir)
        if args.runtime_inp_cache_dir
        else Path(args.out_dir) / "_runtime_inp"
    )
    runtime_inp = _controls_disabled_runtime(
        source_inp=source_inp,
        cache_dir=cache_dir,
        swmm_threads=int(cfg.get("swmm_threads", 1)),
    )
    result = run_authoritative_closed_loop(
        inp_path=runtime_inp,
        output_dir=args.out_dir,
        run_id=args.run_id,
        sensor_nodes=sensors,
        controller=controller,
        control_start_minutes=control_start_minutes,
        control_update_seconds=control_update_seconds,
        observation_update_seconds=model_step_seconds,
        record_stride_seconds=record_stride_seconds,
        exact_global_peak=bool(cfg.get("exact_global_peak", False)),
    )
    print(
        json.dumps(
            {
                "strategy": "proposed",
                "step2_stack": "V7_VALUE_PLUS_V11_HYDRAULIC",
                "runtime_bundle_contract": bundle_meta["runtime_bundle_contract"],
                "development_bundle_allowed": bool(args.allow_development_v110),
                "value_horizon_minutes": 360,
                "hydraulic_horizon_minutes": 120,
                "control_update_seconds": control_update_seconds,
                "first_move_only": True,
                "source_inp": str(source_inp.resolve()),
                "runtime_inp": str(runtime_inp.resolve()),
                "native_controls_enabled": False,
                "metadata_path": result.metadata_path,
                "node_statistics_path": result.node_statistics_path,
                "decisions": result.decisions,
                "global_peak_flood_rate_m3s": result.global_peak_flood_rate_m3s,
                "flow_routing_error_pct": result.flow_routing_error_pct,
            },
            indent=2,
        )
    )


__all__ = ["run_policy_v110_main"]
