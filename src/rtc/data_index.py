...
def build_d2_run_index(manifest: pd.DataFrame, run_summary: pd.DataFrame) -> pd.DataFrame:
    ...
    The D2 center/base action can appear once per actuator in the probe manifest while it is
    physically executed only once. Training on a raw manifest merge would therefore
    overweight that state/action. This compiler collapses those repeated provenance rows
    while checking that event/rainfall/split/checkpoint lineage is invariant.
    """
    ...
    if run_summary.duplicated(keys).any():
        ...
        raise ValueError(f"D2 run summary contains duplicate executed branches: {dup.head().to_dict('records')}")
    joined = run_summary.merge(provenance, on=keys, how="inner", validate="one_to_one", suffixes=("_run", ""))
...
def standardize_d3_run_index(run_summary: pd.DataFrame) -> pd.DataFrame:
    required = {
        ...
        "checkpoint_id", "sequence_sha256", "metadata_path",
    }
...
def build_step2_run_index(
    *,
    ...
    if result["metadata_path"].duplicated().any():
        dup = result.loc[result["metadata_path"].duplicated(False), ["metadata_path", "source_kind"]]
        raise ValueError(f"Step2 index duplicates executed branches: {dup.head().to_dict('records')}")
    ...
    return result.sort_values(
        ["scientific_split", "development_fold", "event_id", "checkpoint_id", "source_kind", "action_or_sequence_sha256"]
    ).reset_index(drop=True)


def main() -> None:
    parser = argparse.ArgumentParser(description="Build one deduplicated lineage-safe D2+D3 Step2 run index")
