...
STATE_SCHEMA = {
    ...
    "state_channels": [
        {"index": 0, "name": "depth", "unit": "m", "online_step1_target": True},
        {"index": 1, "name": "head", "unit": "m", "online_step1_target": True},
        {"index": 2, "name": "flooding_rate", "unit": "m3/s", "online_step1_target": True},
        {"index": 3, "name": "node_volume", "unit": "m3", "online_step1_target": True},
    ],
        ...
        "actuators": [
            {
                ...
                "kind": actuator.kind,
                "upstream_node": actuator.upstream_node,
                "downstream_node": actuator.downstream_node,
                "min_setting": actuator.min_setting,
                "max_setting": actuator.max_setting,
                "continuous": True,
            }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compile frozen Formal RTC physical/model assets")
    parser.add_argument("--inp", required=True)
    parser.add_argument("--priority", required=True)
    parser.add_argument("--sensors", required=True)
    parser.add_argument("--out-dir", required=True)
    args = parser.parse_args()
    payload = compile_assets(
        inp_path=args.inp,
