...
TRAINING_CONTRACT = "STEP2_COUNTERFACTUAL_ACTION_SENSITIVE_TRAINING_V1"
...
def _pair_numpy(ds, left: int, right: int, horizon: int) -> dict[str, np.ndarray]:
    rows = np.asarray([left, right], dtype=int)
    payload = {
        ...
        "target_actuator_flows": ds["target_actuator_flows"][rows, :horizon].astype(np.float32),
        "elapsed_seconds": ds["elapsed_seconds"][rows, : horizon + 1].astype(np.float32),
    }
    if "exact_node_flood_volume_m3" in ds.files:
        payload["exact_node_flood_volume_m3"] = ds["exact_node_flood_volume_m3"][rows].astype(
            np.float32
        )
    return payload


def _to_device(payload: dict[str, np.ndarray], device: torch.device) -> dict[str, torch.Tensor]:
    return {
...
def train_step2_large_v3_main() -> None:
    ...
    parser.add_argument("--precision", choices=("auto", "bf16", "fp32", "fp16"), default="auto")
    ...
    if args.batch_size != 2:
        raise ValueError(
            "counterfactual Step2 uses exactly one same-prefix pair per micro-batch; "
            "--batch-size must remain 2"
        )
    if min(
        args.epochs,
        args.grad_accum,
    ...
    contract_sha, code_sha = training_contract_sha("step2_counterfactual_v3", train_payload)
            ...
            with np.load(str(item["path"]), allow_pickle=False) as ds:
                ...
                for group_key, indices in groups:
                    ...
                    if source == "D3" and not use_d3:
                        continue
                    budget = (
                        args.d3_pairs_per_group if source == "D3" else args.d2_pairs_per_group
                    )
                    for left, right in rotated_reference_pairs(
                        ds,
                        indices,
                        epoch=epoch,
                        budget=budget,
                    ):
                        np_pair = _pair_numpy(ds, left, right, horizon)
                        ...
                        dt = pair["elapsed_seconds"][:, 1:] - pair["elapsed_seconds"][:, :-1]
                        ...
                        totals["total_loss"] += float(full_loss.detach())
                        totals["absolute_state_loss"] += float(metrics.absolute_state.detach())
                        totals["absolute_flow_loss"] += float(metrics.absolute_flow.detach())
                        totals["delta_state_loss"] += float(metrics.delta_state.detach())
                        totals["delta_flow_loss"] += float(metrics.delta_flow.detach())
                        totals["delta_tfv_loss"] += float(metrics.delta_tfv.detach())
                        totals["ranking_loss"] += float(metrics.ranking.detach())
                        totals["exact_flood_loss"] += float(metrics.exact_flood.detach())
                        totals["physical_loss"] += float(metrics.physical.detach())
                        if not flow_only:
                            totals["sensitivity_ratio"] += float(
                                metrics.sensitivity_ratio.detach()
                            )
                            totals["sign_accuracy"] += float(metrics.sign_correct.detach())
