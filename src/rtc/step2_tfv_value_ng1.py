...
def d2_magnitude_weights(values_m3: np.ndarray | list[float], strata: dict[str, Any]) -> np.ndarray:
    values = np.asarray(values_m3, dtype=np.float64).reshape(-1)
    if values.size == 0:
...
class NG1ProcessAwareDirectTFVValueModel(DirectFacilityTFVValueModel):
    ...
    def __init__(
        ...
        *,
        ...
        self.register_buffer("pair_indices", torch.as_tensor(pair_indices, dtype=torch.long))
        self.register_buffer("pair_relation_features", torch.as_tensor(pair_features, dtype=torch.float32))
    ...
    def main_parameter_names(self) -> tuple[str, ...]:
        ...
        return tuple(name for name, _ in self.named_parameters() if name not in interaction)
    ...
    def _joint_latent(
        ...
        context = torch.cat(
            (
                ...
                common,
                blocks,
                state_context[:, None].expand(-1, self.design.actuator_count, -1),
                rainfall_context[:, None].expand(-1, self.design.actuator_count, -1),
            ),
            dim=-1,
        )
