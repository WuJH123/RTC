...
def _verified_lock(path: str | Path) -> dict[str, object]:
    ...
    if lock.get("rtc_source_tree_sha256") != rtc_source_tree_sha256():
        raise ValueError("current implementation contract differs from Policy Lock")
    ...
    if lock.get("formal_metric_aggregation") != "equal_weight_per_independent_rainfall_group":
        raise ValueError("Policy Lock lacks rainfall-group-balanced aggregation")
    ...
    if not isinstance(model_contracts, dict) or not str(model_contracts.get("swmm_engine_version", "")).strip():
        raise ValueError("Policy Lock lacks trained SWMM engine identity")
    return lock
...
def compile_final_v5(
    *, policy_lock_path: str | Path, run_index_path: str | Path
    ...
    if strategies != EXPECTED_STRATEGIES:
        raise ValueError(f"Formal baseline plan must be exactly {list(EXPECTED_STRATEGIES)}")
    ...
    split = split.copy()
    for col in ("event_id", "rainfall_group", "scientific_split"):
    ...
    index = index.copy()
    for col in ("event_id", "rainfall_group", "strategy"):
    ...
    for event, group in index.groupby("event_id", sort=False):
        ...
        if present != expected:
            raise ValueError(f"incomplete seven-strategy Final matrix at {event}: {sorted(present)}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compile complete seven-strategy rainfall-balanced Final")
    parser.add_argument("--policy-lock", required=True)
    parser.add_argument("--run-index", required=True)
    parser.add_argument("--out-dir", required=True)
    args = parser.parse_args()
    detail, summary, pairs = compile_final_v5(
    ...
    _group_detail(detail, metric_cols).to_csv(out / "formal_final_group_detail.csv", index=False)
