...
def _decision_schedule(path: str | Path) -> list[dict[str, object]]:
    ...
    if any(
        int(decisions[i]["elapsed_seconds"]) >= int(decisions[i + 1]["elapsed_seconds"])
...
def replay_exact_global_peak(
    *,
    ...
    """Replay a frozen action schedule at every SWMM routing callback for peak reporting.

    The replay never calls the controller, Step1, Step2 or a rainfall forecast. It uses the
    already-frozen executed decision log from the main causal closed-loop run, so computing
    a reporting metric cannot alter the policy that generated PFV/TFV.
    """
    ...
    for item in schedule:
        ...
        if not isinstance(settings, dict) or set(settings) != expected:
            raise ValueError("replay decision does not specify the complete frozen actuator set")
    ...
    with Simulation(str(inp)) as sim:
        # Deliberately do not call step_advance(): Python observes every routing callback.
        ...
        for _ in sim:
            ...
            while applied < len(schedule) and int(schedule[applied]["elapsed_seconds"]) <= elapsed:
                ...
                applied += 1
            if applied:
                # Re-assert the most recent Python action after native controls at each step.
                settings = schedule[applied - 1]["settings"]
                assert isinstance(settings, dict)
                for aid in catalog.ids:
                    link_obj[aid].target_setting = float(settings[aid])
            total_native = sum(max(0.0, float(obj.flooding)) for obj in node_obj.values())
            peak = max(peak, float(flow_rate_to_m3s(total_native, flow_units)))
        flow_error = float(sim.flow_routing_error)

    if applied != len(schedule):
        raise RuntimeError(f"replay ended before all decisions were applied: {applied}/{len(schedule)}")
    payload: dict[str, object] = {
        "contract": "ROUTING_STEP_GLOBAL_PEAK_REPLAY_V1",
        "inp_path": str(inp.resolve()),
        "inp_sha256": sha256_file(inp),
        "decision_log_path": str(Path(decision_log_path).resolve()) if decision_log_path else None,
        "decision_log_sha256": sha256_file(decision_log_path) if decision_log_path else None,
        "source_main_metadata_path": str(Path(source_main_metadata_path).resolve()) if source_main_metadata_path else None,
        "source_main_metadata_sha256": sha256_file(source_main_metadata_path) if source_main_metadata_path else None,
