"""Bounded V4.4.1 conduit-edge-residual training."""
...
from .step2_control_response_v441 import (
...
TRAINING_CONTRACT_V441 = "PROJECT7_STEP2_EDGE_PHYSICS_CORRECTNESS_V441"
...
def train_conduit_edge_residual_v441(
    *,
    model: DifferentiableCounterfactualResponseModelV441,
    ...
    if not grouped_pairs:
        raise ValueError("V4.4.1 training requires at least one group")
    ...
    if not parameters:
        raise RuntimeError("V4.4.1 selected no trainable edge parameters")
    ...
    for epoch in range(1, int(epochs) + 1):
        ...
        for group in order:
            ...
            if not torch.isfinite(loss):
                raise FloatingPointError(f"non-finite V4.4.1 loss at epoch {epoch}, group {group}")
            ...
            if not np.isfinite(gradient):
                raise FloatingPointError(f"non-finite V4.4.1 gradient at epoch {epoch}, group {group}")
            tick = time.perf_counter()
            optimizer.step()
    ...
    return {
        "contract": TRAINING_CONTRACT_V441,
...
__all__ = ["TRAINING_CONTRACT_V441", "train_conduit_edge_residual_v441"]
