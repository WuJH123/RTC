...
Historical SWMM branch tensors retain authoritative checkpoint state and realised future
rainfall because they were generated by SWMM. Neither is an admissible online Step2 input.
V127 therefore replaces future rainfall with the causal runtime forecast and replaces
checkpoint truth with the frozen Step1 reconstruction from the same sparse causal history.

Raw file SHA values remain provenance only.  Train/deploy compatibility is bound to the
semantic Step1 parameter tensors and ordered sensor IDs, so harmless file reserialization
or comments cannot invalidate an otherwise identical model/input surface.
"""
...
V127_CAUSAL_STATE_CONTRACT = "PROJECT7_V127_CAUSAL_STEP1_STATE_STORE_V2_SEMANTIC_IDENTITY"
...
def semantic_sensor_layout_sha256(nodes: Sequence[str]) -> str:
    payload = json.dumps([str(node) for node in nodes], separators=(",", ":"))
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()


def semantic_model_state_dict_sha256(model: torch.nn.Module) -> str:
    """Hash ordered parameter/buffer tensor semantics, not checkpoint serialization bytes."""
...
class CausalStateStoreV127:
    ...
    sensor_sha256: str
    graph_sha256: str
    step1_model_semantic_sha256: str
    sensor_layout_semantic_sha256: str
    ...
    def validate(self) -> None:
        ...
        for value in (
            ...
            self.sensor_sha256,
            self.graph_sha256,
            self.step1_model_semantic_sha256,
            self.sensor_layout_semantic_sha256,
        ):
            if len(str(value)) != 64:
                raise ValueError("V127 causal state store lacks a canonical lineage fingerprint")
...
def load_causal_state_store_v127(path: str | Path) -> CausalStateStoreV127:
    with np.load(path, allow_pickle=False) as raw:
        required = {
            ...
            "step1_model_semantic_sha256", "sensor_layout_semantic_sha256",
        }
        ...
        result = CausalStateStoreV127(
            ...
            step1_model_semantic_sha256=str(raw["step1_model_semantic_sha256"].item()),
            sensor_layout_semantic_sha256=str(raw["sensor_layout_semantic_sha256"].item()),
